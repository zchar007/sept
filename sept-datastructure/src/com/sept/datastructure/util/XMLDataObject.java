...
public class XMLDataObject {
	...
	public XMLDataObject(String xmlStr) throws DataException {
		try {
	...
	private DataObject parseDataObject() throws DataException, ApplicationException {
		if (null == element) {
		...
		for (Iterator<?> it = element.elementIterator(); it.hasNext();) {
			...
			if (!element1.getName().toUpperCase().equals("P")) {
				throw new DataException("解析xml格式的DataObject时出错,<d>标签中只允许直接出现<p>标签！");
			}
			// DataObject下的元素都有key
			...
			try {
				...
				throw new DataException("解析xml格式的DataObject时出错,【" + key + "】的类型是【" + pdo.getTypeList() + "】必须含有v属性！");
			}
	...
	public DataObject getDataObject() throws DataException, ApplicationException {
