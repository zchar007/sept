...
public class FilterUtil {
	 ...
	 */
	public static HashMap<String, Object> filter(String conditions) throws ApplicationException {
		...
		for (int i = 0; i < expStrArr.length; i++) {
			...
			if (!matcher4ComparisonOperator.find()) {
				throw new ApplicationException(
						"在表达式【" + currentExp + "】中未找到比较运算符【==、<、>、<=、>=、!=、<>、like、isnull、isnotnull】!");
			}
			...
			if (expArr.length == 2) {
				...
				throw new ApplicationException("表达式【" + currentExp + "】结构错误，不符合单目、双目运算表达式结构!");
			}
