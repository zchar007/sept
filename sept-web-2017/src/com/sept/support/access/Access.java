...
import com.sept.support.util.FileIOTool;
/**
 * 创建Access文件 <br>
...
public class Access {
	...
	public Access(String url, Database.FileFormat fileFormat)
			throws AppException {
		try {
			this.url = url;
	...
	public void newTable(String tableName,
			ArrayList<ColumnBuilder> columnBuilders) throws AppException {
		if (hmTables.containsKey(tableName)) {
			throw new AppException("Access文件【" + url + "】已存在名为【" + tableName
					+ "】的表！");
		}
	 ...
	 * @since V1.0
	 */
	public void newTable(String tableName, String columnBuilders)
			throws AppException {
	...
	public void addRow(String tableName, Map map) throws IOException,
			AppException {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public void addRows(String tableName, List list) throws IOException,
			AppException {
		if (!this.hmTables.containsKey(tableName)) {
			throw new AppException("不存在表【" + tableName + "】!");
		}
	 ...
	 * @throws Exception 
	 */
	public DataStore select(String tableName, String filterStr)
			throws Exception {
		...
		return vds.filterModify(filterStr);
	}
	...
	public int delete(String tableName, String filterStr) throws AppException {
		...
		try {
			...
			int j = 0;
			for (Row row : table) {
				DebugManager.println("正在查询第" + (++j) + "行");
				vds.addRow();
				for (int i = 0; i < columns.size(); i++) {
					String key = columns.get(i).getName();
					vds.put(vds.rowCount() - 1, key, row.get(key));
	...
	public static void main(String[] args) throws AppException, AppException,
...
//		 sqlBF.setLength(0);
//		 sqlBF.append("select to_char(ryid) ryid, shbzhm, xm from bisv.person where rownum < 1000");
...
//		 FileIOTool.writeBytesToFile(access.getBytes(), file);
		...
		System.out.println(vds.filterKeep(" ryid == 17092310000010636675").rowCount());
	}
}