...
public class ExcelFactory {
	 ...
	 * @since V1.0
	 */
	public static byte[] getExcelFromJSON(Excel excelColumns,
			String excelJsonStr) throws AppException {
	 ...
	 * @since V1.0
	 */
	public static byte[] getExcelFromXML(Excel excelColumns, String xmlStr)
			throws AppException {
	 ...
	 * @since V1.0
	 */
	public static byte[] getExcelFromDataStore(Excel columns, DataStore datas)
			throws AppException {
		try {
	 ...
	 * 清空输出流 response.setHeader("Content-disposition",
	 * "attachment; filename="+new
	 * String("Book1".getBytes("GB2312"),"8859_1")+".xls");// 设定输出文件头
	 * response.setContentType("application/msexcel");// 定义输出类型 WritableWorkbook
	...
	public static void main(String[] args) throws AppException {
		...
		byte[] bytes = ExcelFactory.getExcelFromDataStore(columns, vds);
		System.out.println(bytes.length);

		String jsonStr = vds.toJSON();
		bytes = ExcelFactory.getExcelFromJSON(columns, jsonStr);
		System.out.println(bytes.length);

		String xmlStr = vds.toXML();
		bytes = ExcelFactory.getExcelFromXML(columns, xmlStr);
		System.out.println(bytes.length);
