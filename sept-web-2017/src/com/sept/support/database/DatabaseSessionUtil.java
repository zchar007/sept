...
public class DatabaseSessionUtil {
	...
	public static String getJdbcUrl() {
		...
		if (ds instanceof com.alibaba.druid.pool.DruidDataSource) {
			com.alibaba.druid.pool.DruidDataSource druidDs = (com.alibaba.druid.pool.DruidDataSource) ds;
	...
	public static int getDBType(String dbName) throws AppException {
		...
		if (ds instanceof com.alibaba.druid.pool.DruidDataSource) {
			...
			if ("POSTGRESQL".equalsIgnoreCase(dbType)) {
				...
				throw new AppException("数据库连接串【" + jdbcUrl + "】中数据库类型【"
						+ dbType + "】目前框架无法识别!");
			}
		}
	...
	public static DataSourceTransactionManager getCurrentTM(String dbName)
		...
		if (!tmMap.containsKey(dbName)) {
			DruidDataSource dataSource = getCtx().getBean(dbName,
	...
	public static void initTransactionManager() throws AppException {
		...
		// 初始化数据源
		DataSource dataSource = ctx.getBean(DEFAULT_DATASOURCE_NAME,
				DataSource.class);
	...
	public static ApplicationContext getCtx() throws AppException {
		...
		DataObject config = GlobalNames.getDeploy("sql");
		dbConfigName = config.getString("SPRING-DBNAME");
		if (ctx == null) {
			if (GlobalNames.CONFIGINWAR) {
				ctx = new ClassPathXmlApplicationContext("classpath:"
						+ dbConfigName);
			} else {
				ctx = new FileSystemXmlApplicationContext(
						GlobalNames.CONFIGFILE + GlobalNames.WEB_APP_NAME + "/"
								+ dbConfigName);
			}
		}
		return ctx;
