...
import com.sept.db.pdf.AbstractDataSpace;
...
public class Excel extends AbstractDataSpace {
	...
	public ExcelSheet addSheet(String sheetName, IDataTableColumns ditColumns) throws AppException {
		...
		dealSheet(sheet, ditColumns);
		ExcelSheet es = new ExcelSheet(sheet, writeAble);
	...
	public ExcelSheet getSheet(int index) throws AppException {
		WritableSheet wsheet = book.getSheet(index);
		if (this.hmSheets.containsKey(wsheet)) {
			return this.hmSheets.get(wsheet);
		}
		ExcelSheet es = new ExcelSheet(wsheet, writeAble);
		this.hmTables.put(wsheet.getName(), es);
		this.hmSheets.put(wsheet, es);
		return es;
	}

	private void dealSheet(WritableSheet sheet, IDataTableColumns ditColumns) throws AppException {
		try {
	...
	@Override
	public IDataTable getTable(String tableName) throws AppException {
		return this.getSheet(tableName);
	}
