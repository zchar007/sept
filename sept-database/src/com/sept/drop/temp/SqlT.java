...
import com.sept.drop.util.BlobUtil;
import com.sept.drop.util.ClobUtil;
...
public class SqlT implements ISql {
	...
	public void setClob(int index, Clob para) throws AppException {
		if (null == para) {
			...
			// 只有在blob流没关闭且blob不是很大的情况下才允许使用
			this.setPara(index, new ClobValue(ClobUtil.clob2string(para)));
		}
	}
	...
	public void setClob(int index, byte[] para) throws AppException {
		if (null == para) {
			...
			this.setPara(index, new ClobValue(new String(para)));
		}
	}
